...
- (void)replyToHTTPRequest
{
	...
	if (httpResponse == nil) {
        // AVPlayer/MPMovieViewController找HttpServer/CocoaHttpServer服务器
        // 要0101-00000001.ts
        // 如果找 /iPhone手机上没有这个文件  我们就要去真正的服务器去下载
        httpResponse = [self httpM3u8ResponseForMethod:method URI:uri];
    }
	if (httpResponse == nil)
	{
		[self handleResourceNotFound];
...
// 这里使用同步下载 要改成异步下载啊
- (NSObject<HTTPResponse> *)httpM3u8ResponseForMethod:(NSString *)method URI:(NSString *)path {
    // path就是 xxx.ts是某个视频的片段
	NSString *filePath = [self filePathForURI:path allowDirectory:NO];
	NSString *dir = [filePath stringByDeletingLastPathComponent];
    NSString *name = [filePath lastPathComponent];
    NSString *newFile = [NSString stringWithFormat:@"%@/%@", dir, name];
    NSLog(@"new file is %@", newFile);
    // /Users/yang/Library/Application Support/iPhone Simulator/6.1/Applications/426D37A7-67CF-48A4-A660-0A59545255F1/Documents/Web/A4CB20C4C3BCDE1834DFA408EAF75749/0101-00000019.ts
    //  http://192.168.88.10/html/ms/friends/0101/  0101-00000019.ts
    
// [config.server realM3U8Url] = http://192.168.88.10/html/ms/friends/0101/0101.m3m8
    NSString *urlstring = [NSString stringWithFormat:@"%@/%@",  [[[config.server realM3U8Url] absoluteString] stringByDeletingLastPathComponent],  name];
    // urlstring = http://192.168.88.10/html/ms/friends/0101/0101-00000019.ts
    // 转化成url
    NSURL *url = [NSURL URLWithString:urlstring];
    // 同步下载, 
    NSData *d = [NSData dataWithContentsOfURL:url];
    
    NSString *tsPath = [NSString stringWithFormat:@"%@/%@", [config.server m3u8LocalDir], name];
    [d writeToFile:tsPath atomically:YES];
    NSLog(@"url is %@ d is %d", url, d.length);
    
    //    NSData *d = [NSData dataWithContentsOfFile:newFile];
    HTTPDataResponse *res = [[HTTPDataResponse alloc] initWithData:d];
    return res;
}
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    NSLog(@"uri path is %@ filepath %@", path, filePath);
	return nil;
}
